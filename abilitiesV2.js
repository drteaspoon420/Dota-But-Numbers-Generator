...
function nozero(key,value) {
    if (key == "AbilityCooldown" || key == "AbilityManaCost") {
        return (value < 1)?1:value;
    }
    return value;
}

function repl(key,content) {
    if (typeof content !== 'object') {
        if (typeof content == 'string') {
            ...
            if (isNaN(Number(sl[0]))) {
                ...
                if (rl.length > 1) {
                    let diff = (sl[2] - sl[1])
                    for (let i = rl.length; i < lvl; i++) {
                        rl[i] = nozero(key,fixNumber((sl[0] + (diff * i))))
                    }
                }
            }
            if (!ok)
...
for (const key in abilities.DOTAAbilities) {
    if (Object.hasOwnProperty.call(abilities.DOTAAbilities, key)) {
        if (!skip_keys.includes(key)) {
            ...
            over.DOTAAbilities[key] = repl(key,element);
            over.DOTAAbilities[key].MaxLevel = lvl;
        }
    }
}
